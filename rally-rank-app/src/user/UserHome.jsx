...
function UserHome() {
    ...
    async function getLeaderboard() {
        try {
            const response = await axios.get("http://localhost:8080/users/home");
    ...
    useEffect(() => {
        const fetchLeaderboard = async () => {
            try {
                setLoading(true);
                const fetchedPlayers = await getLeaderboard();
                    ...
                    {loading ? (
                        ...
                        players.length > 0 ? (
                            players.map((player) => (
                                ...
                                <div
                                    key = { player.username }
                                    className = {`p-2 ${player.username === currentUser.username ? "bg-yellow-200" : "" }`}
                                >
                        ...
                        <div className = "p-2 bg-yellow-200">
                            ...
                            <p> Elo Rating: {currentUser.eloRating} </p>
